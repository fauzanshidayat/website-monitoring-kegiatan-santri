    ...
    <div class="container-fluid mt-4">
        <div class="card">
            <div class="card-header">
                <h4>Edit Data Prestasi Santri</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('prestasi.update', $prestasi->id) }}" method="POST">
                        ...
                        <select name="santri_id" class="form-control @error('santri_id') is-invalid @enderror" required>
                            ...
                            @foreach ($santri as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('santri_id', $prestasi->santri_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama_lengkap }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="tingkat" class="form-control @error('tingkat') is-invalid @enderror" required>
                            ...
                            @foreach (['Sekolah', 'Kecamatan', 'Kabupaten', 'Provinsi', 'Nasional', 'Internasional'] as $tingkat)
                                <option value="{{ $tingkat }}"
                                    {{ old('tingkat', $prestasi->tingkat) == $tingkat ? 'selected' : '' }}>
                                    {{ $tingkat }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" rows="3" class="form-control @error('keterangan') is-invalid @enderror">{{ old('keterangan', $prestasi->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
